...
gen_solinas_matrix(60, 21, 1)


def plot():

    sns.set_theme(style='darkgrid')
    cmap = sns.color_palette('ch:2,r=.2,l=.6')
    ...
    ax = sns.scatterplot(x='bit width', y='mod add/sub', hue='prime', size='prime',
                           sizes=(40, 300), data=df1, palette=cmap)
    plt.ylim(0, 10.5)
    ax.set_xlabel('bit width', fontsize=16)
    ax.set_ylabel('mod add/sub', fontsize=16)
    ax.tick_params(axis='both', which='major', labelsize=16)

    ax.legend(title='num of\nprimes', bbox_to_anchor=(1.02, 0.8), facecolor='white', edgecolor='white')
