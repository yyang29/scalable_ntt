...
def genAddrRom(fileName, dp, sizeN, addrVec, memIdx, module_prefix):
    ...
    with open(fileName, 'a') as wrFile:
        wrFile.write("\n")                    
        wrFile.write("module  "+module_prefix + "_" + addrRomNamePre+"dp")
        wrFile.write(str(dp))
        wrFile.write("_mem")
    ...
    return module_prefix + "_" + addrRomNamePre+"dp"+ str(dp)+ "_mem"+ str(memIdx)


##Generate control module for addr rom
def genAddrGen(fileName, dp, sizeN, addrVec, perIdx, module_prefix):
    ##vecSize = x * sizeN/dp
    romAddrWidth = []
    for i in range(0,dp):
    ...
    for memIdx in range(0, dp):
        addrRomNames.append(genAddrRom(fileName, dp, sizeN, addrVec[memIdx], memIdx, module_prefix))

    #Gen connection module
    with open(fileName, 'a') as wrFile:
        wrFile.write("\n")                    
        wrFile.write("module  "+module_prefix + "_" + addrRomCtrlModuleName+"dp")
        wrFile.write(str(dp))
        wrFile.write("_per")
        ...
        for i in range(0,numWireGroup):
            wrFile.write("reg ["+str(list(sameSizeMap.keys())[i]-1)+":0] rom_addr_"+str(i)+";        \n  ")
    ...
    with open(fileName, 'a') as wrFile:
        ...
        wrFile.write("if(rst) begin                    \n      ")
        for i in range(0, numWireGroup):
            wrFile.write("rom_addr_"+str(i)+" <= "+str(list(sameSizeMap.keys())[i])+"'b0;    \n      ")
    ...
    return module_prefix + "_" + addrRomCtrlModuleName+"dp"+str(dp)+"_per"+str(perIdx)


def genTopMemAddr(fileName, dp, sizeN, addrVec, perIdx, memIdx, module_prefix):
    ...
    with open(fileName, 'a') as wrFile:
        wrFile.write("\n")                    
        wrFile.write("module  "+module_prefix + "_" + addrMemGenModuleName+"dp")
        wrFile.write(str(dp))
    ...
    return module_prefix + "_" + addrMemGenModuleName+"dp"+ str(dp)+ "_mem"+ str(memIdx) + "_per" + str(perIdx)


##Generate optimized control module for addr rom
##Specifically for bit reversal only
def genAddrAutoGen(fileName, dp, sizeN, addrVec, perIdx, module_prefix):
    ##vecSize = x * sizeN/dp
    addrWidth = int(math.ceil(math.log(sizeN/dp, 2.0)))
    ...
    with open(fileName, 'a') as wrFile:
        wrFile.write("\n")                    
        wrFile.write("module  "+module_prefix + "_" + addrMemCtrlModuleName+"dp")
        wrFile.write(str(dp))
        wrFile.write("_per"+str(perIdx))
        wrFile.write("(\n")
        wrFile.write(inStartPortName+",                          \n")
    ...
    return module_prefix + "_" + addrMemCtrlModuleName+"dp"+str(dp)+"_per"+str(perIdx)
...
def getPortStyle(dp, sizeN, addrVec):
    portStyle = []
    incVec = [i for i in range(int(sizeN/dp))]
    for vec in addrVec:
...
def genMemStage(fileName, dp, sizeN, stride, dataWidth, addrVec, regOut, perIdx, module_prefix):
    ...
    #for memIdx in range(0, dp):
    if stride != 0:
        addrGenModuleName = genAddrGen(fileName, dp, sizeN, addrVec, perIdx, module_prefix)
    else:
        addrGenModuleName = genAddrAutoGen(fileName, dp, sizeN, addrVec, perIdx, module_prefix)
    ...
    with open(fileName, 'a') as wrFile:
        wrFile.write("\n")   
        moduleName = module_prefix + "_" + memStageName + str(dp) ##??+  
        if(regOut):
            moduleName += "_r"     
