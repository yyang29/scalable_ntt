...
def mapStridePer(inSize, inIdx, stride):
    ...
    outIdx = (int)(inIdx*stride) % (inSize) + (int)(stride*inIdx/inSize)
    return outIdx


#Generate a stage of switches based on ctrl bits
def genSwitchesStage(fileName, dp, lOrR, dataWidth, regOut, nullVec, stageIdx, perIdx, module_prefix):
    #Gen 2to2 switch module
    #gen2to2Switch(fileName, dataWidth, False)
    moduleName = module_prefix + "_switches_stage_st"+str(stageIdx)+"_"+str(perIdx)+"_"+lOrR
...
def genCtrlRom(fileName, stageIdx, dp, lOrR, ctrlVec, perIdx, module_prefix):
    vecSize = len(ctrlVec)  ##vecSize = dp/2
    addrWidth = int(math.ceil(math.log(len(ctrlVec[0]), 2.0)))  #addrWidth = log2(input size/(dp/2))
    moduleName = module_prefix + "_ctrl_rom_dp"+str(dp)+"_st"+str(stageIdx)+"_per"+str(perIdx)+lOrR
...
def genSwitchCtrl(fileName, stageIdx, dp, lOrR, ctrlVec, perIdx, module_prefix):
    vecSize = len(ctrlVec)
    addrWidth = int(math.ceil(math.log(len(ctrlVec[0]), 2.0)))
    moduleName = module_prefix + "_switch_ctrl_dp"+str(dp)+"_st"+str(stageIdx)+"_per"+str(perIdx)+lOrR
    #print "debug:",str(perIdx)

    nameCtrlRom = genCtrlRom(fileName, stageIdx, dp, lOrR, ctrlVec, perIdx, module_prefix)
...
def genStagesBlock(fileName, dp, g_stride, g_latency, lOrR, dataWidth, ctrlIn3D, regOutSwitch, regOutWireCon, regOut, perIdx, module_prefix):
    ...
    ##Get switch stage
    swithStageNames = ["" for k in range(int(math.log(dp,2)))]
    switchesCtrlNames = ["" for k in range(int(math.log(dp,2)))]
    wireConStageNames = ["" for k in range(int(math.log(dp,2)))]

    ##calcualte number of cycles for 0 or 1
    cycle_period_vec = [0 for i in range(int(math.log(dp,2)))]

    for stageIdx in range(0, int(math.log(dp,2))):
        #cycyle_period
        start = ctrlIn3D[stageIdx][0][0]
        count = 1
        while count < len(ctrlIn3D[stageIdx][0]) and ctrlIn3D[stageIdx][0][count] == start: 
            count += 1
        cycle_period_vec[stageIdx] = count if count < len(ctrlIn3D[stageIdx][0]) else len(ctrlIn3D[stageIdx][0])/2

        #nullVec: size of dp/2, 0:a switch, 1: cross state, 2:pass state
        nullVec = [0 for i in range(int(dp/2))]
        for i in range(0,int(dp/2)):
        ...
        swithStageNames[stageIdx] = genSwitchesStage(fileName, dp, lOrR, dataWidth, regOutSwitch[stageIdx], nullVec, stageIdx, perIdx, module_prefix)
        ##Generate control for swiches
        if g_stride != 0:
            switchesCtrlNames[stageIdx] = genSwitchCtrl(fileName, stageIdx, dp, lOrR, ctrlIn3D[stageIdx], perIdx, module_prefix)
        ##Generate wire connection stages
        wireConStageNames[stageIdx] = genWireCon(fileName, stageIdx, dp, lOrR, dataWidth, regOutWireCon[stageIdx], module_prefix)

    tmpStr = "ingress" if(lOrR == "L") else "egress"

    moduleName = module_prefix + "_" + tmpStr + "Stage_p" + str(dp)
    #Gen connection module
    with open(fileName, 'a') as wrFile:
        wrFile.write("\n")                    
        wrFile.write("module " + moduleName)
    ...
    with open(fileName, 'a') as wrFile:
        wrFile.write(";\n  ")
        width = int(math.log(g_latency,2.0))
        wrFile.write("input ["+str(width-1)+":0] counter_in; \n  ")
    ...
    with open(fileName, 'a') as wrFile:
        ...
        for i in range(0, int(math.log(dp,2))):
            ...
            if (lOrR == "L"):
                ...
                wrFile.write("reg ["+str(int(dp/2-1))+":0] wire_ctrl_stage"+str(stageIdx)+";\n\n  ")
            ...
            if g_stride == 0:
                wrFile.write("\n  ")
                if i == 0:
                    wrFile.write("wire ["+str(int(math.log(g_latency,2))-1)+":0] counter_w;\n  ")
                    wrFile.write("assign counter_w = counter_in; \n  ")
                ...
                for i in range(dp/2):
                    if lOrR == "L":
                        ...
                        wrFile.write("always@(posedge clk)             \n  ")
                        wrFile.write("begin                            \n    ")
                        wrFile.write("wire_ctrl_stage"+str(stageIdx)+"["+str(i)+"] <= counter_w["+str(width_c)+"]; \n  ")
                        wrFile.write("end                            \n\n  ")
            else:
                wrFile.write("\n  ")
